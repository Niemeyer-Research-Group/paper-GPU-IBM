...
Both methods use the fractional-step approach~\cite{Perot1993} to solve Equations \eqref{eq:NavierStokes} and \eqref{eq:Continuity}, which breaks up the discretization of the Navier--Stokes equations into three sub-steps.
...
Final hybrid node values are a weighted combination of both solutions where the weighting is designed to smooth the transition between solutions. 
...
Its purpose is to dampen the numerical oscillations that result from the discontinuous regime switch. 
...
Sections~\ref{sec:ID external}, and \ref{sec:ID embedded} cover the modifications required for the presence of an immersed boundary with the modified Fadlun, external, and embedded methods. 
